...
- **Performance Optimization** — OpenMP-accelerated matrix multiplication
...
```bash
gcc -O2 -fopenmp matrix.c test.c -o test
./test
```

### Build benchmark (if benchmark.c exists)

```bash
gcc -O2 -fopenmp matrix.c benchmark.c -o benchmark
OMP_NUM_THREADS=4 ./benchmark openmp "OpenMP optimization"
```
